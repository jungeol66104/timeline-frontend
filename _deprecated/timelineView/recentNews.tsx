...
//
//     return (
//         <div className={`flex flex-col ${recentNews.length <= 0 && 'hidden'}`}>
...
//                 const isLatest = article.live
//
//                 const handleLinkClick = () => window.open(article.url, '_blank')
//
//                 return (
//                     <div key={article.id} onClick={handleLinkClick} className={`cursor-pointer px-3 py-2 mb-3 border-[1px] ${isLatest ? 'border-red-300' : 'border-gray-300'} rounded-lg hover:bg-gray-100 ${!isToggle && i !== 0 && 'hidden'}`}>
//                         {isLatest
//                             ?   <div className={`flex gap-1.5 items-center`}><span className={'font-semibold text-sm text-red-700'}>Latest</span><div className={'h-1.5 w-1.5 rounded-full bg-red-700'}></div></div>
