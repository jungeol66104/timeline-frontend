        ...
        <div className={'top-0 right-0 shrink-0'} style={{width: size, height: size}}>
            {isBaseImage
                ? <><div className={'relative w-full h-full rounded-sm bg-gray-500 text-white flex items-center justify-center text-sm font-medium'}>
                        ...
                        <Image src={`/images/base-image/base-image${mapStrToNum(timeline.name)}.jpg`} alt={'base-image'} width={250} height={250} priority={true} className={'rounded-sm'}/>
                    </div></>
                : <Image className={'rounded-xl'} src={timeline.image} alt={timeline.name} width={250} height={250}/>
            }
        </div>
    );
};
