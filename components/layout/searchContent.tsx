    ...
    return (
        <Link href={ tab === 'timeline' ? `/timelines/${searchResult.id}` : `/events/${searchResult.id}`}>
            <div className={'flex items-center pt-[6px] pb-[6px] gap-2.5'}>
                {tab === 'timeline' && <div className={'searchImage w-[28px] h-[28px] relative'}><Image className={'rounded-sm'} src={`/images/timeline/small/${searchResult.id}.png`} alt={`${searchResult.name}`} width={28} height={28} priority={true}/></div>}
