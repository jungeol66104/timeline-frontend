...
const MobileSearchBar = () => {
    ...
    return (
        <div ref={searchBodyRef} className={`absolute top-[10px] right-1.5 max-w-[480px]`} style={{zIndex: 4999, width: `calc(100% - 16px)`}}>
            <div
                className={`h-[40px] w-full px-2.5 flex gap-2.5 items-center border-gray-200 ${isSearch ? 'bg-white rounded-t-lg border-t-[1px] border-x-[1px]' : 'bg-gray-100 rounded-lg border-[1px]'}`}>
                ...
                <div className={'flex w-full'}>
                    ...
                    <div className={'flex items-center gap-2'}>
                        ...
                        <button onClick={() => dispatch(updateIsSearch(false))}
                                className={'material-symbols-outlined text-[24px] shrink-0 flex items-center justify-center rounded-full bg-white'}>&#xe5cd;</button>
                    </div>
                </div>
            </div>
