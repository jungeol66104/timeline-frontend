        ...
        <Link href={`/timelines/${searchResult.id}`} className={'flex items-center py-1.5 gap-2.5'}>
            <div className={'relative shrink-0 w-[28px] h-[28px]'}>
                {isBaseImage
                    ...
                    :   <Image className={'rounded-sm bg-gray-100'} src={"https://" + searchResult.cdnUrl + searchResult.imagePath} alt={searchResult.title} fill={true} priority={true} style={{objectFit: "cover", objectPosition: "top"}}/>}
            </div>
