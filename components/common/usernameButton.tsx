...
const UsernameButton = ({user} : {user: any}) => {
    ...
    return (
        <Link href={`/@${username}`} className={`${timelineType === 'demo' && 'pointer-events-none'} flex items-center justify-center gap-2.5 px-2 h-[36px] border-[0.1px] border-gray-300 bg-white hover:bg-gray-100 ${timelineType === 'demo' && demoKeyConcept === 'private' && 'outline outline-2 outline-blue-700'} drop-shadow-sm rounded-md`}>
