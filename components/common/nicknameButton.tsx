...
const NicknameButton = ({name} : {name: string}) => {
    ...
    return (
        <Link href={'/'} className={`flex items-center gap-2.5 px-3 max-[852px]:px-2 h-[36px] border-[0.1px] border-gray-300 bg-white hover:bg-gray-100 ${timelineType === 'demo' && demoKeyConcept === 'private' && 'outline outline-2 outline-blue-700'} drop-shadow-sm rounded-md`}>
            <div className={'flex gap-1.5 justify-center items-center'}>
                ...
                <span className={`${timelineType === 'demo' && 'max-[852px]:hidden'} text-sm font-semibold`}>{name}</span>
            </div>
        </Link>
    );
};
