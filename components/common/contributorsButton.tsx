...
const ContributorsButton = () => {
    ...
    return (
        <div className={'relative'}>
            <button ref={contributionButtonRef} onClick={handleClick} className={`flex items-center px-3 max-[852px]:px-2 h-[36px] border-[0.1px] border-gray-300 bg-white hover:bg-gray-100 drop-shadow-sm rounded-md`}>
                <div className={'text-sm font-semibold max-[852px]:hidden'}>Contributors</div>
                <div className={'relative h-[26px] shrink-0 max-[852px]:hidden'} style={{width: `${Math.min(contributors.length - 1, 4) * (26 - 7) + 26 + 10}px`}}>
                ...
                <div className={'relative h-[26px] shrink-0 min-[852px]:hidden'} style={{width: `${Math.min(contributors.length - 1, 1) * (26 - 7) + 26}px`}}>
                    {contributors.map((contributor, i) => {
                        if (i > 1) return
                        const initial = contributor.substring(0, 2).toUpperCase();

                        return (
                            <div key={i} className={'absolute top-0 w-[26px] h-[26px] rounded-full flex items-center justify-center bg-gray-600 text-white text-xs border-[1px] border-white shrink-0'} style={{left: `${i * (26 - 7)}px`}}>{initial}</div>
                        )
                    })}
                </div>
