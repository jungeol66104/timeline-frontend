...
const ToolbarExpanded = () => {
    ...
    let zIndex = -4999
    if (toolbarStatus === "expand") zIndex = 4999
    return (
        <div draggable onDragStart={(e) => handleDragStart(e)} onDragEnd={handleDragEnd} onDragEnter={(e) => e.preventDefault()} onDragOver={(e) => e.preventDefault()} className={`${isToolbarDrag ? 'opacity-0' : ''} fixed bottom-[25px] left-1/2 transform -translate-x-1/2 flex items-center justify-center w-[200px] h-[40px] border-[1px] rounded-3xl bg-white drop-shadow-md`} style={{zIndex: zIndex}}>
